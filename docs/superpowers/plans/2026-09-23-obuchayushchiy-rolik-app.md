...
## Изменения после съёмки (2026-09-23)

Первая партия кадров показала, что инструменты, книги и поездки пусты. Ролик
сокращён, см. спеку, раздел «Что пришлось сократить». Что это меняет в плане:

- **`TOTAL_FRAMES` = 1260**, не 1800. Ролик 42 секунды, семь сцен.
- **Тип `Scene`:** вариант `full` убран (сцены с проездом не осталось), вариант
  `zoom2` теперь ссылается на **один** скриншот и два прямоугольника:
  `{ kind: 'zoom2'; shot: string; title: string; frames: number; focus: [Rect, Rect] }`.
- **Задача 9 (`SceneFull`) отменена.** Компонент вернётся, когда разделы
  наполнятся. `panOffset` остаётся — `SceneZoom` берёт из неё базовое положение
  камеры.
- **Кадров пять**, не восемь: `01-zakrep`, `02-glavnyy`, `03-yarmarka`,
  `04-obshchiy-dom`, `05-sosedi`. Задача 13 размечает пять, а не шесть.
- **Сценарий в задаче 11** — из семи сцен по таблице раскадровки в спеке.
- В `public/shots/` уже лежат **черновые** кадры: пережатые Telegram снимки,
  конвертированные в PNG. На них отлаживается всё, кроме резкости; оригиналы
  кладутся поверх теми же именами.
...
```
длина композиции = сумма(frames) − 12 × (сцен − 1) = 1332 − 72 = 1260
```

Первая сцена входящего перехода не имеет, поэтому её 120 кадров идут как есть.
Остальные шесть получают по +12 к экранному времени.
