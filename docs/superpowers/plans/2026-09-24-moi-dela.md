...
**Отклонения при выполнении (2026-09-24) — код расходится с текстом задач ниже, верен код:**
- Задачи 5–7: вместо широких `listByFamily()`, `listByBorrower()`, `listByOrganizer()`/`listByParticipant()` — узкие запросы `listRejectedByFamily()`, `listOverdueForBorrower()`, `listOpenByOrganizer()`, `listArrivedUnpaidForParticipant()`. Причина — список дел считается на каждой странице, а широкие выборки тянули всю историю семьи (у дневников — с полными текстами). Актуальное описание — в спеке, раздел «Устройство».
- Тесты: всякая проверка «дела нет» подкреплена в том же тесте проверкой, что источник работает (дело было до действия или рядом есть контрольный пример) — сбои источников глотаются, и без этого тесты проходили бы впустую. Отсюда отличия в тестах задач 3, 4, 7, 8, 11 от текста плана.
- Добавлено сверх плана: память списка привязана к жителю; тесты границы «сегодня» у поездок и закупок; тест «организатору не „заберите“ у себя»; тесты экранирования пояснения и блока на главной; сброс кешей в `make_test_db()`.
