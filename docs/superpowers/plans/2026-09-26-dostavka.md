...
Локально PHP нет. Тесты запускаются на сервере `abconsult` по **закоммиченному** состоянию (`git archive HEAD`). Поэтому в каждой задаче сначала коммит, потом прогон; если прогон упал — исправление отдельным коммитом.

Полный прогон:

```bash
cd /c/Projects/skaz-kray-astro
git archive HEAD:residents | ssh abconsult 'rm -rf /root/ledger-test && mkdir -p /root/ledger-test && tar -x -C /root/ledger-test'
ssh abconsult 'cd /root/ledger-test && php8.3 /root/composer.phar install --no-interaction -q && php8.3 vendor/bin/phpunit'
```

Один тест-класс: в конце вместо `vendor/bin/phpunit` — `vendor/bin/phpunit --filter DeliveryPolicyTest`.

(`rm -rf` каталога нужен, чтобы в копии не оставались удалённые файлы.)
