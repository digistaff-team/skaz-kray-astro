...
(−300 см от нуля), опасная отметка 36.160 м (−200 см). Пороги цвета панели
(спокойно / внимание / тревога) — наши, 200 и 50 см запаса, правятся там же.
...
группу жителей — в Telegram (`telegram.group_chat_id`) и в MAX (`max.group_chat_id`).
...
| Событие | Текст |
|---|---|
| спокойно → внимание (запас < 2 м) | ⚠️ Шебш поднимается: до нижней кромки моста … |
| внимание → тревога (запас ≤ 0,5 м) | 🚨 Вода у моста: до нижней кромки … |
| вода выше кромки | 🚨 Мост под водой: уровень выше нижней кромки на … |
| тревога держится дольше 6 часов | тот же текст ещё раз |
| вернулось в спокойное | ✅ Вода отступила: до нижней кромки моста … |

Пока обстановка не меняется, cron молчит. На первом запуске о спокойной воде не
пишем — сообщение уйдёт только если вода уже подошла к мосту. Скачок уровня больше
3 м за шаг считается сбоем источника: оповещения нет, в лог идёт строка (у AllRivers
уже менялась привязка шкалы, ложная тревога хуже пропущенной записи в журнале).
